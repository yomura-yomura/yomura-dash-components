...
__all__ = ["Tail", "filepath_dict", "get_component_id_list"]
...
class Tail(html.Div):
    def __init__(
        ...
        style: dict[str, str]
        | ComponentUndefined = dash.development.base_component.Component.UNDEFINED,
        *,
        filepath: FilePath,
    ):
...
def get_component_id_list(base_id: Optional[ComponentID]) -> list[ComponentID]:
    ...
    if isinstance(base_id, dict):
        ...
        return [
            {first_key: component_id, **{key: base_id[key] for key in other_keys}}
